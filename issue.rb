    ...
    if (u = attrs.delete('assigned_to_id')) && safe_attribute?('assigned_to_id')
      ...
      if u.count == 1
        ...
        if u.count == 2
          ...
          u = u.drop(1)
          usr = u
          u.each do |usuario_id|
            usuario_id = usuario_id.to_i
          end
          logins = []
          nombres = User.select(:login).where(:id => u)
          nombres.each do |nm|
            logins << nm.login
          end
          nombres_usr = logins.join(",")
          if Group.exists?(lastname: nombres_usr)
            @grupo = Group.where(lastname: nombres_usr).first
            @miembro = Member.edit_membership("", {"project_id" => self.project_id, "role_ids" => ["4"]}, @grupo.becomes(Principal))
            @miembro.save
            if assignable_users.any?{|assignable_user| assignable_user.id == @grupo.id}
              self.assigned_to_id = @grupo.id
            end
          else
            @grupo = Group.new
            @grupo.safe_attributes = {"name" => nombres_usr}
            @grupo.save 
            @usuarios = User.not_in_group(@grupo).where(:id => usr).to_a
            @grupo.users << @usuarios
            @miembro = Member.edit_membership("", {"project_id" => self.project_id, "role_ids" => ["4"]}, @grupo)
            @miembro.save
            if assignable_users.any?{|assignable_user| assignable_user.id == @grupo.id}
              self.assigned_to_id = @grupo.id
            end
